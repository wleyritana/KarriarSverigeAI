...
def run():
    ...
    # allow list too
    hard_gates_normalized = hard_gates_data if hard_gates_data else hard_gates_raw
    hard_gate_status = compute_hard_gate_status(hard_gates_normalized)

    deep = requirement_intelligence(job, role, lang)

    ats = ats_audit(cv, job, role, lang)
    psyche = recruiter_psychology(cv, job, role, lang)
    optimized = optimize_cv(cv, job, role, lang)
    ats_cv = ats_submission(cv, job, role, lang)
    interview = interview_pack(cv, job, role, lang)
    culture_report = culture_analysis(company, culture, reviews, lang)

    match_raw = recruiter_match(cv, job, role, lang, json.dumps(hard_gates_data or {}, ensure_ascii=False))
